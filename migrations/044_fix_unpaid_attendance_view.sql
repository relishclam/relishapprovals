-- Safe repair: ensure all objects from 043 exist, then recreate the view.
-- Run this if Labour Dues shows "No unpaid dues" even after marking attendance.

-- 1. Worker-type rates table (idempotent)
...
-- 2. worker_type column on construction_workers (idempotent)
...
SELECT
  ...
  -- Use per-worker-type rate if approved; fall back to supervisor-level rate; then NULL.
...
LEFT JOIN construction_worker_rates wr
 ...
 AND wr.worker_type  = w.worker_type
