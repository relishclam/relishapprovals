-- Migration 013: Suspense/Attachments tables (if not yet created) + Fix capture_sessions FK
-- This migration is self-contained: it safely creates the tables from migration 012
-- if they don't exist yet, then fixes the attachment_id FK to use ON DELETE SET NULL.
-- Previously the FK had no delete action (defaults to RESTRICT), which silently
-- blocked deletion of attachments uploaded via the QR/Send-to-Phone capture flow.

-- ─── Prerequisites from migration 012 (safe to re-run with IF NOT EXISTS) ───
...
CREATE TABLE IF NOT EXISTS capture_sessions (
  ...
  used_at         TIMESTAMPTZ,
  attachment_id   UUID  -- FK added below with ON DELETE SET NULL
);
...
BEGIN
  ...
  UPDATE voucher_series SET suspense_current_number = v_next_number WHERE company_id = p_company_id;
  v_serial := 'SUS-' || COALESCE(v_series.financial_year, '2025-26') || '-' || LPAD(v_next_number::TEXT, 5, '0');
  RETURN v_serial;
END;
$$ LANGUAGE plpgsql;

-- ─── Fix: capture_sessions.attachment_id FK → ON DELETE SET NULL ─────────────

ALTER TABLE capture_sessions
  DROP CONSTRAINT IF EXISTS capture_sessions_attachment_id_fkey;

ALTER TABLE capture_sessions
  ADD CONSTRAINT capture_sessions_attachment_id_fkey
    FOREIGN KEY (attachment_id)
    REFERENCES voucher_attachments(id)
    ON DELETE SET NULL;
